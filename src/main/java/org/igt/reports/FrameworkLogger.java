...
/**
 * Logger to set the status of the log Mar 31, 2022
 * 
...
public class FrameworkLogger {
	...
	public static final Consumer<String> PASS = (message) -> ExtentManager.getExtenttest()
	...
	private static final Consumer<String> INFO = (message) -> ExtentManager.getExtenttest()
			.info(MarkupHelper.createLabel(message, ExtentColor.BLUE));
	...
	public static void log(LogType status, String message) {
		/**
		 * if(!PropertyUtils.getValue(String.valueOf(PropertiesType.PASSEDSTEPSSCREENSHOT)).equalsIgnoreCase("yes"))
		 * { map.getOrDefault(status,Pass).accept(message); } else{
		 * SCREENSHOTMAP.getOrDefault(status,Pass).accept(message); }
		 **/
